...
export const listMyBasket = () => async (dispatch) => {
  const { token } = JSON.parse(localStorage.getItem("userInfo"));
  const { basketId } = jwt_decode(token);
...
export const addToBasketUnknownUser =
  (itemId, setBeer) => async (dispatch, getState) => {
    try {
      // console.log("getState", setBeer);
      const beer = {
        itemId: itemId,
        quantity: setBeer[itemId],
      };

      const data = getState().basketState.itemInBasket
        ? [...getState().basketState.itemInBasket, beer]
        : [beer];

      dispatch({ type: UNKNOWN_USER_BASKET_CREATE_REQUEST });
      dispatch({
        type: UNKNOWN_USER_BASKET_CREATE_SUCCESS,
        payload: data,
      });
    } catch (error) {
      dispatch({
        type: UNKNOWN_USER_BASKET_CREATE_FAIL,
        payload: error,
      });
    }
  };
